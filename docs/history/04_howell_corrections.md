...
The archived emulator implements a pressure expression using `134.7` and says
in a code comment that the value is “per Howell.” That exact emulator behavior
is available only as `modern_powley.later.emulator.pressure_cup`; it does not
verify Howell's publication or promote the expression to a Howell baseline.
